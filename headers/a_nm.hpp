...
template <muint N, muint M> struct A_NM_1 {
	declare (N <= M) ? 0 : (M + 1) * A_NM_1<N - 1, M>::n + (N - M) * A_NM_1<N - 1, M - 1>::n;
};
template <muint N> struct A_NM_1 <N, N> { declare 0; };
template <muint N> struct A_NM_1 <N, 0> { declare 1; };
template <> struct A_NM_1 <0, 0> { declare 1; };

template <muint N, muint M> superconst muint a_nm_1 = A_NM_1<N, M>::n;
...
template <muint N, muint M> struct A_NM_2 {
	declare (N <= M) ? 0 : (2 * N - M - 1) * A_NM_2<N - 1, M - 1>::n + (M + 1) * A_NM_2<N - 1, M>::n;
};
template <muint N> struct A_NM_2 <N, N> { declare 0; };
template <muint M> struct A_NM_2 <0, M> { declare 0; };
template <> struct A_NM_2 <0, 0> { declare 1; };

template <muint N, muint M> superconst muint a_nm_2 = A_NM_2<N, M>::n;
